...
(() => {
  ...
  class ImageSlot extends HTMLElement {
    ...
    constructor() {
      ...
      this._spill.addEventListener('pointerdown', (e) => {
        ...
        if (corner) {
          ...
          // ancestor â€” deck_stage renders slides scaled-to-fit.
      ...
      // Wheel zoom stays available inside reframe mode as a trackpad nicety â€”
      // zooms toward the cursor (offset' = cursorÂ·(1-k) + offsetÂ·k).
    ...
    connectedCallback() {
      // Warn once per page â€” an id-less slot works for the session but
      ...
      // width%/height% in _applyView encode the frame aspect at call time â€”
      // a host resize (responsive grid, pane divider) would stretch the
      // image until the next _render. Re-render on size change: _render()
      // re-seeds _view from stored before clamp/apply, so a shrinkâ†’grow
    ...
    // host's dirty signals â€” announce enter/exit so the host can hold
    ...
    handleEvent(e) {
      if (e.type === 'dragenter' || e.type === 'dragover') {
        ...
        // dragenter/leave fire for every descendant crossing â€” count depth
    ...
    async _ingest(file) {
      ...
      // resumes â€” bump + capture a generation so stale encodes bail.
      const gen = ++this._gen;
      // Replacing a shown image: surface the swap through the encode too,
      // not just the decode â€” otherwise the old photo sits there with no
      ...
      try {
        ...
        // Only exit reframe once the new image is in hand â€” a rejected type
        ...
        if (gen !== this._gen) return;
        this._swapGen = 0;
        // Reveal the kept old image â€” unless another replacement (a
    ...
    // Reframing (pan/resize) is available on any filled slot â€” the user can
    ...
    //  - no encode is pending (_swapGen) â€” mid-encode the stored value
    //    still resolves to the old image, so any reveal paints it;
    //  - the frame img has settled on its current src â€” an unsettled src
    ...
    // branch (the img is being cleared â€” nothing will ever settle).
    ...
    // box â€” ResizeObserver fires with a 0Ã—0 rect under display:none, and
    // clamping against a degenerate 1Ã—1 frame would silently pull the stored
    // pan toward zero.
    _geom() {
    ...
    _applyView() {
      ...
      // (the same 8px inset as the in-frame layout; unscaled â€” top-layer UI
      ...
      // shove it off-frame â€” and matches(':popover-open') itself throws
      ...
      if (!g) {
        // Dimensions not known yet (before img load) â€” centered fit so there
      ...
      // Baseline (cover-fill or contain-fit) Ã— view scale. Width/height and
      // left/top are all frame-% â€” depends only on the frame aspect ratio, so
      ...
      if (this.hasAttribute('data-reframe')) {
        ...
        // origin AND size, and the rect/layout ratio rescales the ghost â€”
    ...
    _render() {
      ...
      // tool, so its value isn't guaranteed canvas-originated â€” only accept
      ...
      // only value must count as missing â€” otherwise it would suppress the
      ...
      if (url && !attrError) {
        ...
        if (prev !== url) {
          ...
          // placeholder-until-load behavior â€” no spinner. _hidShowing
          ...
          // Same-src re-render â€” release if settled, so an ingest-set
        ...
        this.removeAttribute('data-swapping');
        // The src is being removed â€” no load/error will ever fire for it.
        this._loadPending = false;
      ...
      if (showCredit) {
        ...
        // Unsplash's prescribed credit is TWO links â€” the photographer's
